...
namespace AlfredInterface.Controllers.Gesture
{
    public class GestureEventHandlers
    {
        ...
        private static double _humanInactivitySeconds = 0;
        private DateTime _lastSkeletonDate = DateTime.Now;
        private readonly LightModel _salon;

        public GestureEventHandlers(GestureController controller)
        {
            string _apiUrl = "http://api-nam.kicks-ass.org/device/";
            var httpClient = new HttpClient();
            var lights = JsonConvert.DeserializeObject<IEnumerable<LightModel>>(httpClient.GetStringAsync(_apiUrl + "list").Result);

            _salon = lights.SingleOrDefault(d => d.Name == "Salon");
        ...
        public void intStream_InteractionFrameReady(object sender, InteractionFrameReadyEventArgs e)
        {
            ...
            var curUsers = usrInfo.Where(x => x.SkeletonTrackingId > 0).ToList<UserInfo>();
